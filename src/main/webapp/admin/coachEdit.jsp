...
<body>
	<!-- 增加的模态框 -->
	<div class="modal fade" id="coachAddModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	...
	<!-- 更新的模态框 -->
	<div class="modal fade" id="coachUpdateModal" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
					<button type="button" class="btn btn-primary" id="coach_update_btn">更新</button>
				</div>
			</div>
		</div>
	</div>
...
$(function() {
	$.ajax({
		url:"${APP_PATH}/getCoachs",
		type:"get",
		success:function(res){
			build_mans_panels(res.extend.coachs);
		}
	});
})
...
$(document).on("click",".edit_btn",function(){
	...
	//3、把id传递给模态框的更新按钮
	$("#coach_update_btn").attr("edit-id",$(this).attr("edit-id"));
	$("#coachUpdateModal").modal({
		backdrop:"static"
	});
});
...
$("#coach_update_btn").click(function() {			
	...
	$.ajax({
		...
		type:"PUT",
		data:$("#coachUpdateModal form").serialize(),
		success:function(result){
			alert("更新成功！");
			//1、关闭对话框
			$("#coachUpdateModal").modal("hide");
			//2、回到本页面
			location.reload();
		}
	});
...
$("#man_add_btn").click(function() {
	...
	$.ajax({
		url:"${APP_PATH}/coach",
		type:"POST",
		data:$("#coachAddModal form").serialize(),
		success:function(result){
			if(result.code == 200){
				//录入成功
				//关闭模态框
				$('#coachAddModal').modal('hide');
				//发送ajax请求显示最后一页数据即可
				location.reload();
			}
		}
